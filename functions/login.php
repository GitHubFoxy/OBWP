...
$email = $_POST['email'];
$sql = "SELECT * FROM `laba2_users` WHERE login = '$login' AND email = '$email'";
    ...
    if(password_verify($password ,$row['password'])) 
        {
            ...
            echo '<form class="flex flex-col gap-4" method="POST" action="./update.php"><input class="hidden" name="id" value="'. $row['id'] .'"><div class="flex gap-2 justify-between items-center"><label for="login">Login</label><input class="p-2 rounded-md bg-slate-200" type="text" name="login" placeholder="' .$row['login'] . '"></div><div class="flex gap-2 justify-between items-center"><label for="password">Password</label><input name="password" class="p-2 bg-slate-200 rounded-md"type="text" placeholder="'.$password .'"></div><button class="px-4 py-2 bg-slate-300 rounded-lg transition-colors hover:bg-slate-400" type="submit">Edit</button></form>';
            echo '</div>';
        }
    else 
        {
